...
function logoutFarivote() {
	//clear localstorage
	localStorage.removeItem("saveItems");
}

function prepareFarivotesAfterLogin() {
	//clear localstorage
	localStorage.removeItem("saveItems");

	// make new data
	ajaxRequest("/util/favorites", "GET", null, function(res) {
		console.log(res);
		for (var i = 0; i < res.length; i++) {
			var item = res[i];
			saveToLocalStorage(item.title, item.urlPost, item.thumnail, item.description, item.address, item.formality, item.categoryId, item.newsId);
		}
		window.location = "/";
	});
}
